...
def write_split_csvs(site_data_list, output_folder='data_exports'):
    ...
    with open(river_file, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=river_headers)
        writer.writeheader()
        writer.writerows(river_rows)
    print(f"✅ Wrote {len(river_rows)} rows to {river_file}")

    with open(weather_file, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=weather_headers)
        writer.writeheader()
        writer.writerows(weather_rows)
    print(f"✅ Wrote {len(weather_rows)} rows to {weather_file}")
